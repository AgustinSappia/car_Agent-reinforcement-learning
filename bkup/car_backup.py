...
class Car:
    ...
    def __init__(self, environment):
        ...
        self.x = self.env.width / 2  # Posición inicial X (centro horizontal, escalable)
        self.y = self.env.height * 0.75  # Posición inicial Y (inferior de la pista, escalable)
    ...
    def _cast_ray(self, track, rel_angle):
        ...
        x, y = self.x, self.y
        # Usar int() para range() (evita error con floats)
        max_dist = int(self.max_sensor_distance)
        ...
        return max_dist  # No se detectó nada (usar int para consistencia)
    ...
    def draw_sensors(self, screen):
        """
        Dibuja los sensores como líneas verdes desde el auto hasta el punto detectado.
        """
        for i, (rel_angle, dist) in enumerate(zip(self.sensor_angles, self.sensor_distances)):
            ...
            # Dibujar línea (verde para visibilidad)
